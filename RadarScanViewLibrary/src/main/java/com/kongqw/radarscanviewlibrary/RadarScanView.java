...
public class RadarScanView extends View {
    ...
    private static int anInt,anInt1;
    // 雷达扫描一圈的时间 默认1秒转一圈
    ...
    private static final int DEFAULT_RADAR_BACKGROUND_LINES_COLOR = Color.BLUE;
    ...
    public static Paint mRadarBackgroundLinesPaint;
    ...
    private Paint mRadarScanPaint,mRadarScanPaint1;
    private Paint mRadarBackgroundPaint;
    private int mViewRadius;
    private float mScanRadius;
    public static Canvas canvas;
    private ArrayList<Integer> objects,objects1;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawFan(canvas, mViewRadius);
         if (people==true){
        random = new Random();
        anInt = random.nextInt(963 - 25) + 25;
        anInt1 = random.nextInt(((630-500)+477 * 2) - (630-500)) + (630-500);
            objects.add(anInt);
            objects1.add(anInt1);
        people=false;
        }
        ...
        if (objects.size()>0){
            mMatrix1.postRotate(-360.0F / mRadarScanTime * REFRESH_RATE, mCenterPoint.x, mCenterPoint.y);
            canvas.concat(mMatrix1);
            for (int i =0;i<objects.size();i++){
                 canvas.drawCircle(objects.get(i), objects1.get(i),10,mRadarScanPaint1);
                canvas.rotate(0);
...
//        //480 630
//        random = new Random();
//        anInt = random.nextInt(963 - 3) + 3;
//        anInt1 = random.nextInt(((630-477)+630 * 2) - (630-477)) + (630-477);
//        canvas.drawCircle(anInt, anInt1,20,mRadarBackgroundLinesPaint);
//    }

        public static void addpeople(boolean people1){
            people = people1;
    }
