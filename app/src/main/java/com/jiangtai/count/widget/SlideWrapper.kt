package com.jiangtai.count.widget
...
class SlideWrapper : RelativeLayout {
    ...
    private fun doAnimation() {
        ...
        var dx = 0
        var close = true
        val viewConfiguration = ViewConfiguration.get(App.getMineContext())
        val touchSlop = viewConfiguration.scaledTouchSlop
        val scrolledX = (mXMove - mXDown).roundToInt()
        if (scrolledX > touchSlop) {
            dx = -scrollX
            mNotifyPullback = !mNotifyCompleteClose
        } else {
