...
public class SharedPreferanceUtils {
    ...
    public static final String FILE_NAME1 = "share_data1";
    ...
    public static void putImage(Context mContext, String key, ImageView imageView) {
        ...
        // 将String保存shareUtils
        SharedPreferanceUtils.put(mContext, key, imgString);
    }
    ...
    public static Bitmap getImage(Context mContext, String key, ImageView imageView) {
        String imgString = (String) SharedPreferanceUtils.get(mContext, key, "");
        if (!imgString.equals("")) {
            // 利用Base64将我们string转换
    ...
    public static <T> void setDataList(Context context, String tag, List<T> datalist) {
        SharedPreferences sp = context.getSharedPreferences(FILE_NAME1,
                Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = sp.edit();
        if (null == datalist)
            return;
