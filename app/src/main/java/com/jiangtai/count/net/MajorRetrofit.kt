 ...
 */
class MajorRetrofit private constructor() {
    //private var serverIp = "http://169.254.28.220:8082/dcmp/"
...
//                        CacheControl.Builder builder = new CacheControl.Builder().maxAge(10, TimeUnit.MINUTES);
    ...
    companion object {
        private var mService: MajorRetrofit? = null
        private const val DEFAULT_TIME_OUT = 10 //超时时间
        private const val DEFAULT_READ_TIME_OUT = 10
        val instance: MajorRetrofit
            get() {
                if (mService == null) {
                    synchronized(MajorRetrofit::class.java) {
                        if (mService == null) {
                            mService = MajorRetrofit()
                        }
                    }
                }
                return mService!!
    ...
    init {
        ...
        val retrofit = Retrofit.Builder()
            ...
            .build()
        api = retrofit.create(MajorApi::class.java)
    }
}